...
switch ($ac) {
    case "grabar":
        $p = new puntosDao();
        $exito = $p->grabar($_POST["nombre"], $_POST["lat"], $_POST["lng"]);
        if($exito)
        {
            $r["estado"] = "ok";
    ...
    case "borrar":
        $p = new puntosDao();
        $resultados = $p->borrar($_POST["idestacion"]);
        if($resultados)
        {
            $r["estado"] = "ok";
    ...
    case "actualizar":
        $p = new puntosDao();
        $exito = $p->actualizar($_POST["id"], $_POST["nombre"], $_POST["lat"], $_POST["lng"]);
        if($exito)
        {
            $r["estado"] = "ok";
    ...
    default:
     $r["estado"] = "error";
     $r["mensaje"] = "datos no válidos estado por default";
     break;
}
